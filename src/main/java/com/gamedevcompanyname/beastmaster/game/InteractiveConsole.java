...
import com.gamedevcompanyname.beastmaster.game.Elements.Player;

import java.util.List;

import static com.gamedevcompanyname.beastmaster.game.Config.MAX_LINE_LENGHT;
...
public class InteractiveConsole {
    ...
    private final String[] moves = new String[]{"Отдохнуть", "Исследовать", "Осмотреться", "Путешествовать *№*",
                                                "Персонаж", "Статистика", "Выход", "Взаимодействовать", "Локация", "Объект"};
    private final MagicExchangerInterface pray;
    ...
    public InteractiveConsole(MagicExchangerInterface pray){
        this.pray = pray;
        /*
        for (int i = 0; i < 100; i++){
    ...
    private boolean goTo() {
        ...
        do {
            ...
            for (int i = 0; i < choose.length(); i++){
                if (!Character.isDigit(choose.charAt(i))) isCorrect = false;
            }
            int index = -1;
    ...
    private void say(String wordOfGod){
        ...
        for (int i = 0; i < wordOfGod.length(); i++){
            pray.say(String.valueOf(wordOfGod.charAt(i)));
            sleep(12);
            count++;
